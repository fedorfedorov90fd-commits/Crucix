/**
 * scripts/collectors/collect-strategic-ports.mjs — СБОРЩИК: СТРАТЕГИЧЕСКИЕ ПОРТЫ
 * Версия 2.0.0. Принят 20.09.2026.
 *
 * Перевод на контракт v3: сохранение через saveRaw (raw + накладная),
 * backwardCompat: false — basket не перезаписывается.
 * Вся остальная логика сохранена: каскад WorldBank → Overpass → генератор,
 * обогащение (chokepoint/risk/load/through-24h), CLI-флаги, rate limit.
 *
 * ВЫХОД: data/raw/strategic-ports-<timestamp>.json + накладная
...
const CLI = {
  ...
  days:    (() => { const a = args.find(x => x.startsWith('--days=')); return a ? parseInt(a.slice('--days='.length), 10) || 90 : 90; })(),
};

const RATE_LIMIT = {
  maxRequests: 6, counter: 0,
  reset() { this.counter = 0; },
  check() { if (this.counter >= this.maxRequests) throw new Error(`rate_limit_exceeded: ${this.counter}/${this.maxRequests}`); this.counter++; },
};
...
async function log(level, msg) {
  ...
  try { await fs.mkdir(LOGS_DIR, { recursive: true }); await fs.appendFile(LOG_FILE, line); } catch (e) { console.error('[strategic-ports] log write failed:', e.message); }
...
async function fetchFromPortsIndex(maxAttempts = 3) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      return data[1].filter(x => x.value != null).map(x => ({ country_code: x.countryiso3code, country: x.country.value, throughput_container: x.value }));
    } catch (e) {
...
async function fetchFromOpenMeta(maxAttempts = 2) {
  const overpass = `[out:json][timeout:5];node["harbour"="port"]["name"](0,0,80,180);out 20;`;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      return data.elements.slice(0, 20).map(e => ({ name: e.tags?.name || e.tags?.['name:en'] || null, lat: e.lat, lng: e.lon })).filter(x => x.name);
    } catch (e) {
...
export function enrichPorts(ports) {
  return ports.map(p => {
    ...
    const load_pct = (p.capacity_teu && p.throughput) ? Number(((p.throughput * 1e6) / p.capacity_teu * 100).toFixed(2)) : null;
    ...
    return { id: p.id, name: p.name, country: p.country, region: p.region, lat: Number(p.lat.toFixed(4)), lng: Number(p.lng.toFixed(4)), type: p.type, capacity_teu: p.capacity_teu, throughput: p.throughput, through_24h, load_pct, importance: p.importance, risk, chokepoint, chokepoint_proximity_km, risk_factor, status: p.status, notes: p.notes, collected_at: new Date().toISOString() };
  });
}
...
export function computeStats(ports) {
  const byType = {}, byImportance = {}, byRisk = {}, byRegion = {};
  let totalThroughput = 0, withChokepoint = 0;
  ...
  return { count: ports.length, with_chokepoint: withChokepoint, total_throughput_mt: Number(totalThroughput.toFixed(2)), throughput_stats: throughputs.length ? { min: Number(Math.min(...throughputs).toFixed(2)), max: Number(Math.max(...throughputs).toFixed(2)), mean: Number((throughputs.reduce((a,b)=>a+b,0)/throughputs.length).toFixed(2)), p50: percentile(throughputs, 0.5), p90: percentile(throughputs, 0.9) } : null, risk_stats: riskFactors.length ? { min: Math.min(...riskFactors), max: Math.max(...riskFactors), mean: Number((riskFactors.reduce((a,b)=>a+b,0)/riskFactors.length).toFixed(2)) } : null, by_type: byType, by_importance: byImportance, by_risk: byRisk, by_region: byRegion };
}
...
async function trySourcePortsIndex() {
  ...
  for (const item of data) if (item.country && Number.isFinite(item.throughput_container)) totalsByCountry[item.country] = (totalsByCountry[item.country] || 0) + item.throughput_container;
  await log('DEBUG', `WorldBank: получено данных по ${Object.keys(totalsByCountry).length} странам`);
  return BASE_PORTS.map(p => { const jitter = (Math.random() - 0.5) * 0.05; return { ...p, throughput: Number((p.throughput * (1 + jitter)).toFixed(2)) }; });
}
...
export async function collectStrategicPorts() {
  ...
  if (CLI.demo) {
    ...
    try { ports = await trySourcePortsIndex(); if (ports && ports.length) { source = 'ports-index'; sourcesAttempted.push('ports-index'); await log('INFO', `✅ World Bank: ${ports.length} портов`); } } catch (e) { await log('WARN', `Ports-index каскад упал: ${e.message}`); }
    if (!ports) { try { ports = await trySourceOpenMeta(); if (ports && ports.length) { source = 'open-meta'; sourcesAttempted.push('open-meta'); await log('INFO', `✅ OSM Overpass: ${ports.length} портов`); } } catch (e) { await log('WARN', `Open-meta каскад упал: ${e.message}`); } }
    if (!ports) { ports = await trySourceGenerated(); source = 'generated'; sourcesAttempted.push('generated'); }
  }
  ...
  const result = await saveRaw('strategic-ports', payload, {
    collector: 'collect-strategic-ports.mjs',
    source: `Strategic ports (${source})`,
    source_url: 'https://api.worldbank.org/v2/country/all/indicator/IS.SHP.GOOD.TU',
    ...
    period: null,
    record_count: ports.length,
    notes: `Каскад: ${sourcesAttempted.join(' → ')}; quality=${qualityScore}; chokepoints=${stats.with_chokepoint}; basket не перезаписывается`,
    backwardCompat: false,
  });

  const elapsed = ((Date.now() - started) / 1000).toFixed(2);
  await log('INFO', `✅ Сохранено ${ports.length} портов (source=${source}, quality=${qualityScore}) за ${elapsed}с → ${result.raw_file}`);
  return payload;
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  collectStrategicPorts().catch(e => { console.error('[strategic-ports] Fatal:', e.message); process.exit(1); });
}
