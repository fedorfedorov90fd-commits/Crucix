/**
 * apis/sources/big-mac-alt-api.mjs — API-МОДУЛЬ: ИНДЕКС БИГ-МАКА (АЛЬТЕРНАТИВНЫЙ)
 *
 * КОНТРАКТ CRUCIX v2.
 * ИСТОЧНИК: data/basket/big-mac-alt.json — { type:'FeatureCollection', features:[{ type:'Feature', properties:{ name, value, label:'price', unit:'$' } }] }.
 * Сборщик: scripts/collectors/collect-big-mac-alt.mjs.
 *
 * Альтернативный индекс Биг-Мака (по странам). Геокоординаты добавляются из встроенного справочника.
 *
...
const BASKET_FILE  = join(PROJECT_ROOT, 'data', 'basket', 'big-mac-alt.json');

export const route  = '/api/layers/big-mac-alt';
...
export const meta = {
  ...
  color: '#ff8800',
  vizType: 'choropleth',
  source: 'basket/big-mac-alt.json',
  collector: 'collect-big-mac-alt.mjs',
  cache: 300,
  description: 'Альтернативный индекс Биг-Мака',
  unit: 'USD',
};
...
async function loadData() {
  ...
  try { raw = await fs.readFile(BASKET_FILE, 'utf8'); }
    ...
    if (e.code === 'ENOENT') {
      ...
      err.hint = 'run scripts/collectors/collect-big-mac-alt.mjs'; throw err;
    }
    throw e;
  }
  let parsed;
...
function extractRows(doc) {
  if (Array.isArray(doc)) return doc.map((r, i) => ({ name: r.country || r.name, value: Number(r.price ?? r.value), date: r.date || null, i }));
  if (doc && Array.isArray(doc.features)) {
    return doc.features.map((f, i) => {
      const p = f.properties || {};
      return { name: p.name || p.country, value: Number(p.value ?? p.price), date: p.date || null, unit: p.unit || '$', label: p.label || 'price', i };
    });
  }
  return [];
}
  ...
  const features = rows
    ...
    .map(r => {
      ...
      return {
        ...
        properties: {
          name: r.name, value: r.value, unit: r.unit || '$', label: r.label || 'price', date: r.date,
...
export async function handler(req, res) {
  try {
    ...
    const sub = urlObj.pathname.replace(/^\/api\/layers\/big-mac-alt/, '') || '/';
