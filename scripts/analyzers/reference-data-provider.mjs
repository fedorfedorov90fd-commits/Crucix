...
import ReferenceDataProvider from '../../apis/sources/reference-data-provider.mjs';
const __dirname = dirname(fileURLToPath(import.meta.url));
const ROOT = join(__dirname, '..', '..');
const OUT_DIR = join(ROOT, 'data', 'analytics', 'specialist');
const OUT_FILE = join(OUT_DIR, 'reference-data-provider.json');
async function main() {
  const now = new Date().toISOString();
  const t0 = Date.now();
  const instance = new ReferenceDataProvider();
  let extra = {};
  ...
  const payload = {
    _meta: { id: 'reference-data-provider', category: 'specialist', version: '1.0.0', schema_version: '1.0.0', sources: ['self'], calculator: 'ReferenceDataProvider', updated_at: now, duration_ms: Date.now() - t0, checksum: '', stats },
  ...
  console.log('reference-data-provider v1.0.0 — stats:', JSON.stringify(stats));
}
