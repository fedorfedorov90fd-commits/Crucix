/**
 * apis/sources/threat-classification-api.mjs — API-МОДУЛЬ: КЛАССИФИКАЦИЯ УГРОЗ
 *
 * КОНТРАКТ CRUCIX v2.
 * ИСТОЧНИК: data/analytics/detector/threat-classification.json.
 * Анализатор: scripts/analyzers/threat-classification.mjs.
 *
 * Классификация угроз по типу, критичности, источнику (acled, cyber-attacks).
 */
...
const ANALYTICS_FILE = join(PROJECT_ROOT, 'data', 'analytics', 'detector', 'threat-classification.json');

export const route  = '/api/layers/threat-classification';
...
export const meta = {
  ...
  color: '#a21caf',
  vizType: 'marker',
  source: 'analytics/detector/threat-classification.json',
  collector: 'analyzer:threat-classification',
  cache: 300,
  description: 'Классификация угроз по типу и критичности',
  unit: 'threats',
};
...
async function loadData() {
  ...
  try { raw = await fs.readFile(ANALYTICS_FILE, 'utf8'); }
    ...
    if (e.code === 'ENOENT') {
      ...
      err.hint = 'run scripts/analyzers/threat-classification.mjs'; throw err;
    }
    throw e;
  }
...
function extractItems(data) {
  ...
  if (data.data && Array.isArray(data.data.threats)) return data.data.threats;
  if (data.data && Array.isArray(data.data.classified)) return data.data.classified;
...
function computeStats(rows) {
  ...
  const bySeverity = {}, byType = {}, bySource = {};
  for (const r of rows) {
    bySeverity[r.severity] = (bySeverity[r.severity] || 0) + 1;
    byType[r.type] = (byType[r.type] || 0) + 1;
    if (r.source) bySource[r.source] = (bySource[r.source] || 0) + 1;
  }
  const top_types = Object.entries(byType).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([name, count]) => ({ name, count }));
  return { count: rows.length, by_severity: bySeverity, by_type: byType, top_types };
}
...
function toFeatureCollection(rows) {
  const features = rows.filter(r => r.lat !== 0 || r.lng !== 0).map(r => ({
    ...
    properties: {
      ...
      score: r.score, source: r.source, sourceType: r.sourceType,
      description: r.description, region: r.region, date: r.date,
...
function toCSVBody(rows) {
  const lines = ['id,name,type,severity,score,source,region,date'];
  for (const r of rows) lines.push(`${r.id},"${r.name.replace(/"/g, '""')}",${r.type},${r.severity},${r.score},${r.source || ''},${r.region || ''},${r.date || ''}`);
  return lines.join('\n') + '\n';
}
...
export async function handler(req, res) {
  try {
    ...
    const extra = { 'X-Module': 'threat-classification-api', 'X-Module-Version': '2.0.0', 'Cache-Control': `public, max-age=${meta.cache}` };
