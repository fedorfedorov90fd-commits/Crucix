/**
 * scripts/collectors/collect-wti-brent-spread.mjs — СБОРЩИК: СПРЕД WTI-BRENT
 * Версия 2.0.0. Принят 20.09.2026.
 *
 * Перевод на контракт v3: saveRaw (raw + накладная), backwardCompat: false.
 * Merge с basket заменён на merge с in-memory (mergeWithExisting больше не читает basket).
 * Вся логика сохранена: каскад Yahoo → FRED → генератор, rolling stats,
 * RSI, regime, rate limit, CLI-флаги.
 *
 * ВЫХОД: data/raw/wti-brent-spread-<timestamp>.json + накладная
 */
...
const VALIDATION = { wtiRange: [20, 200], brentRange: [20, 200], spreadRange: [-5, 30], minPoints: 5, maxPoints: 365 };
const FRED_SERIES = { wti: 'DCOILWTICO', brent: 'DCOILBRENTEU' };
...
async function log(level, msg) {
  ...
  try { await fs.mkdir(LOGS_DIR, { recursive: true }); await fs.appendFile(LOG_FILE, line); } catch (e) { console.error('[wti-brent-spread] log write failed:', e.message); }
...
function percentile(arr, p) { if (!arr.length) return null; const sorted = [...arr].sort((a, b) => a - b); const idx = Math.floor((sorted.length - 1) * p); return sorted[idx]; }
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      for (let i = 0; i < timestamps.length; i++) {
        const v = closes[i];
        if (v == null) continue;
        points.push({ date: new Date(timestamps[i] * 1000).toISOString().slice(0, 10), value: Math.round(v * 100) / 100 });
      }
...
function generateFallback(days = 30) {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    data.push({ date: daysAgo(i), value: Math.round(spread * 100) / 100, wti: Math.round(wti * 100) / 100, brent: Math.round(brent * 100) / 100, spread: Math.round(spread * 100) / 100 });
  }
  return data;
}
...
export function mergeTickers(wtiPoints, brentPoints) {
  ...
  for (const w of wtiPoints) {
    ...
    merged.push({ date: w.date, value: spread, wti: w.value, brent: b, spread });
  }
...
export function computeRollingStats(points, window = 7) {
  ...
  return { window, samples: tail.length, mean: Number(mean.toFixed(3)), stddev: Number(stddev.toFixed(3)), min: Number(Math.min(...tail).toFixed(3)), max: Number(Math.max(...tail).toFixed(3)), ema: Number(ema.toFixed(3)), p25: percentile(tail, 0.25), p75: percentile(tail, 0.75) };
}
...
function trySourceGenerated() { return generateFallback(CLI.days); }

export async function collectWtiBrentSpread() {
  await log('INFO', '🚀 Запуск сборщика wti-brent-spread (v2.0.0)');
  ...
  if (CLI.demo) {
    ...
    try { data = await trySourceYahoo(); if (data) { source = 'Yahoo Finance'; sourcesAttempted.push('yahoo'); await log('INFO', `✅ Yahoo Finance: ${data.length} точек`); } } catch (e) { await log('WARN', `Yahoo каскад упал: ${e.message}`); }
    if (!data) { try { data = await trySourceFred(); if (data) { source = 'FRED'; sourcesAttempted.push('fred'); await log('INFO', `✅ FRED: ${data.length} точек`); } } catch (e) { await log('WARN', `FRED каскад упал: ${e.message}`); } }
    if (!data) { data = trySourceGenerated(); source = 'generated'; sourcesAttempted.push('generated'); await log('WARN', `Все live-источники недоступны, fallback: ${data.length} точек`); }
  }

  data = validatePoints(data);
  if (!data.length) throw new Error('no_valid_points_after_validation');
  data = dedupeByDate(data).slice(-VALIDATION.maxPoints);
  ...
  const result = await saveRaw('wti-brent-spread', payload, {
    collector: 'collect-wti-brent-spread.mjs',
    source: `WTI-Brent spread (${source})`,
    source_url: 'https://query1.finance.yahoo.com/v8/finance/chart/CL=F',
    license: 'public-domain',
    format_hint: 'timeseries',
    value_type: 'price',
    value_unit: 'usd_per_bbl',
    granularity: 'daily',
    period: 'P30D',
    record_count: data.length,
    notes: `Каскад: ${sourcesAttempted.join(' → ')}; RSI=${rsi}; regime=${regime}; quality=${qualityScore}; basket не перезаписывается`,
    backwardCompat: false,
  });

  const elapsed = ((Date.now() - started) / 1000).toFixed(2);
  await log('INFO', `✅ Сохранено ${data.length} точек (source=${source}, quality=${qualityScore}) за ${elapsed}с → ${result.raw_file}`);
  return payload;
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  collectWtiBrentSpread().catch(e => { console.error('[wti-brent-spread] Fatal:', e.message); process.exit(1); });
}
