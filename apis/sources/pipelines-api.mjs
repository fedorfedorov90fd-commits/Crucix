/**
 * apis/sources/pipelines-api.mjs — API-МОДУЛЬ: ТРУБОПРОВОДЫ
 *
 * КОНТРАКТ CRUCIX v2.
 * ИСТОЧНИК: data/basket/pipelines.json — массив { id, name, lat, lng, severity, timestamp }.
 * Сборщик: scripts/collectors/collect-pipelines.mjs.
 *
 * Трубопроводы — ключевая энергетическая инфраструктура. Уязвимы к диверсиям.
 */
...
const BASKET_FILE  = join(PROJECT_ROOT, 'data', 'basket', 'pipelines.json');

export const route  = '/api/layers/pipelines';
...
export const meta = {
  ...
  color: '#ff8800',
  vizType: 'marker',
  source: 'basket/pipelines.json',
  collector: 'collect-pipelines.mjs',
  cache: 600,
  description: 'Трубопроводы — ключевая энергетическая инфраструктура',
  unit: 'nodes',
};
...
async function loadPipelines() {
  ...
  try { raw = await fs.readFile(BASKET_FILE, 'utf8'); }
    ...
    if (e.code === 'ENOENT') {
      ...
      err.hint = 'run scripts/collectors/collect-pipelines.mjs'; throw err;
    }
    throw e;
  }
  let parsed;
  ...
  else if (parsed && Array.isArray(parsed.pipelines)) arr = parsed.pipelines;
...
function computeStats(rows) {
  ...
  return { count: rows.length, by_severity: bySeverity, by_type: byType, by_country: byCountry };
}
...
function toFeatureCollection(rows) {
  const features = rows.map(r => ({
    ...
    properties: {
      ...
      capacity: r.capacity, country: r.country, timestamp: r.timestamp,
      color: SEVERITY_COLOR[r.severity] || SEVERITY_COLOR.INFO,
      category: 'infrastructure', icon: meta.icon,
    },
  }));
  return {
...
function toCSVBody(rows) {
  const lines = ['id,name,type,severity,country,capacity,lat,lng'];
  for (const r of rows) lines.push(`${r.id},"${r.name.replace(/"/g, '""')}",${r.type},${r.severity},${r.country || ''},${r.capacity ?? ''},${r.lat},${r.lng}`);
  return lines.join('\n') + '\n';
}
...
export async function handler(req, res) {
  try {
    ...
    const full = await loadPipelines();
    const rows = applyFilters(full, query);
    const stats = computeStats(rows);
    const extra = { 'X-Module': 'pipelines-api', 'X-Module-Version': '2.0.0', 'Cache-Control': `public, max-age=${meta.cache}` };
