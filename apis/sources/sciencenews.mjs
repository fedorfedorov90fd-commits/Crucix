...
export async function briefing() {
  try {
    const response = await fetch('https://www.sciencenews.org/feed', {
      headers: {
    ...
    console.error(`[Science News] Загружено ${items.length} новостей`);
    ...
    console.error('[Science News] Ошибка:', error.message);
    return {
      source: 'Science News',
      timestamp: new Date().toISOString(),
...
if (process.argv[1]?.endsWith('sciencenews.mjs')) {
