// Quanta Magazine — наука и математика
export async function briefing() {
  try {
    const response = await fetch('https://api.quantamagazine.org/feed/', {
      headers: {
    ...
    console.error(`[Quanta Magazine] Загружено ${items.length} новостей`);
    ...
    console.error('[Quanta Magazine] Ошибка:', error.message);
    return {
      source: 'Quanta Magazine',
      timestamp: new Date().toISOString(),
...
if (process.argv[1]?.endsWith('quanta.mjs')) {
