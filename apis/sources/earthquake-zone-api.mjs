/**
 * apis/sources/earthquake-zone-api.mjs — API-МОДУЛЬ: ЗОНЫ ЗЕМЛЕТРЯСЕНИЙ
 *
 * КОНТРАКТ CRUCIX v2.
 * ИСТОЧНИК: data/basket/earthquake-zone.json — массив { label, value, country, lat, lng }.
 * Сборщик: scripts/collectors/collect-earthquake-zones.mjs.
 *
 * Сейсмоопасные зоны мира. Value 1-5 — уровень риска.
 * Отличие от earthquakes-api: earthquake-zone — это СТАТИЧЕСКИЙ список ЗОН, earthquakes — это фактические события.
 */
...
const BASKET_FILE  = join(PROJECT_ROOT, 'data', 'basket', 'earthquake-zone.json');

export const route  = '/api/layers/earthquake-zone';
...
export const meta = {
  ...
  color: '#ff6600',
  vizType: 'marker',
  source: 'basket/earthquake-zone.json',
  collector: 'collect-earthquake-zones.mjs',
  cache: 3600,
  description: 'Сейсмоопасные зоны мира (статический справочник)',
  unit: 'risk_level',
};
...
async function loadZones() {
  ...
  try { raw = await fs.readFile(BASKET_FILE, 'utf8'); }
    ...
    if (e.code === 'ENOENT') {
      ...
      err.hint = 'run scripts/collectors/collect-earthquake-zones.mjs'; throw err;
    }
    throw e;
  }
  let parsed;
...
export async function handler(req, res) {
  try {
    ...
    const extra = { 'X-Module': 'earthquake-zone-api', 'X-Module-Version': '2.0.0', 'Cache-Control': `public, max-age=${meta.cache}` };
