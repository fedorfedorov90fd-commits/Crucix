...
import MigrationFlowTracker from '../../apis/sources/migration-flow-tracker.mjs';
const __dirname = dirname(fileURLToPath(import.meta.url));
const ROOT = join(__dirname, '..', '..');
const BASKET = join(ROOT, 'data', 'basket');
const OUT_DIR = join(ROOT, 'data', 'analytics', 'flow');
const OUT_FILE = join(OUT_DIR, 'migration-flow-tracker.json');
async function readJson(p, fb = null) { try { return JSON.parse(await readFile(p, 'utf-8')); } catch { return fb; } }
async function main() {
  const now = new Date().toISOString();
  const t0 = Date.now();
  const instance = new MigrationFlowTracker();
  const data = await readJson(join(BASKET, 'refugees.json'), null);
  ...
  const payload = {
    _meta: { id: 'migration-flow-tracker', category: 'flow', version: '1.0.0', schema_version: '1.0.0', sources: ['refugees.json'], calculator: 'MigrationFlowTracker', updated_at: now, duration_ms: Date.now() - t0, checksum: '', stats: { items_processed: count, total_result: all.length } },
  ...
  console.log('migration-flow-tracker v1.0.0 — items:', count, ', result:', all.length);
}
