...
export async function handleCISAAPI(req, res) {
  ...
  try {
    ...
    if (path === '/api/cisa/status' && req.method === 'GET') {
      ...
      res.end(JSON.stringify({
        ...
        module: 'cisa-kev',
        status: 'online',
        vulnerabilities: basket.data.vulnerabilities?.length || 0,
    ...
    res.end(JSON.stringify({
      success: false,
      error: 'Нет данных. Запустите: node scripts/collect-cisa.mjs'
    }));
    return;
  }
