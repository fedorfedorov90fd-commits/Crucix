#!/usr/bin/env node
/**
 * Crucix Collector: geo-data (справочник 192 стран + 14 маркеров + 8 слоёв).
 * Версия 2.0.0. Принят 20.09.2026.
 * Роль: справочник гео-данных → saveRaw. Сборщик НЕ пишет в basket.
 * Формат: {timestamp, version, countries:{...}, markers:[...], layers:{...}, totals:{...}}.
 * Тип — hierarchical (вложенная структура).
 */
...
export async function collectGeoData() {
  ...
  const data = {
    ...
    markers: MARKERS,
    layers: LAYERS,
    totals: { countries: Object.keys(COUNTRIES).length, markers: MARKERS.length, layers: Object.keys(LAYERS).length },
  };
  console.log(`  Стран: ${data.totals.countries}, маркеров: ${data.totals.markers}, слоёв: ${data.totals.layers}`);

  const result = await saveRaw('geo-data', data, {
    collector: 'collect-geo-data.mjs',
    source: 'Crucix geo reference',
    ...
    period: null,
    record_count: data.totals.countries + data.totals.markers + data.totals.layers,
    notes: 'Справочник 192 стран + 14 маркеров + 8 слоёв; basket не перезаписывается',
    backwardCompat: false,
  });
  console.log(`[Geo Collector] OK → ${result.raw_file}`);
  return data;
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  collectGeoData().catch((e) => { console.error('[Geo Collector] FATAL:', e); process.exit(1); });
}
