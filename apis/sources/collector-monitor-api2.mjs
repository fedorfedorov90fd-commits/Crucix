...
// ============================================================
// Исправлена ошибка: resolveProjectRoot() теперь синхронная
// ============================================================
...
function safeReadFileSync(filePath) {
    try {
        return fs.readFileSync ? fs.readFileSync(filePath, 'utf8') : null;
    } catch {
        return null;
    }
}
...
function detectMode(collectorName) {
    ...
    try {
        const content = fs.readFileSync ? fs.readFileSync(scriptPath, 'utf8') : '';
...
function parseLogErrorsSync(logPath) {
    try {
        const content = fs.readFileSync ? fs.readFileSync(logPath, 'utf8') : '';
