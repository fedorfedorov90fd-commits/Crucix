/**
 * apis/sources/recession-api.mjs — API-МОДУЛЬ: ВЕРОЯТНОСТЬ РЕЦЕССИИ
 *
 * КОНТРАКТ CRUCIX v2.
 * ИСТОЧНИК: data/basket/recession.json — [{ date, value }] ИЛИ { source, lastUpdated, data:[{date,value}] }.
 * Сборщик: scripts/collectors/collect-recession.mjs.
 *
 * Вероятность рецессии (0–100%). Обычно рассчитывается на основе кривой
 * доходности, спредов, экономических индикаторов (US Federal Reserve / NY Fed).
 *
 * ФОРМАТЫ: json (FC + series + stats), csv, series, stats, raw.
 * ФИЛЬТРЫ: ?since=, ?until=, ?min=, ?max=, ?tier=, ?limit=.
 *
 * СЛУЖЕБНЫЕ ПОДПУТИ:
 *   GET /stats /status /latest /tiers /trend /composite /featurecollection
 */
...
const BASKET_FILE  = join(PROJECT_ROOT, 'data', 'basket', 'recession.json');

export const route  = '/api/layers/recession';
...
export const meta = {
  ...
  color: '#ff4400',
  vizType: 'series',
  source: 'basket/recession.json',
  collector: 'collect-recession.mjs',
  cache: 300,
  description: 'Вероятность рецессии (0–100%)',
  unit: 'percent',
};

const TIERS = {
  critical: { min: 70, color: '#dc2626', label: 'Критическая' },
  high:     { min: 50, color: '#f97316', label: 'Высокая' },
  elevated: { min: 35, color: '#eab308', label: 'Повышенная' },
  moderate: { min: 20, color: '#84cc16', label: 'Умеренная' },
  low:      { min: 0,  color: '#22c55e', label: 'Низкая' },
};
...
async function loadData() {
  ...
  try { raw = await fs.readFile(BASKET_FILE, 'utf8'); }
    ...
    if (e.code === 'ENOENT') {
      ...
      err.hint = 'run scripts/collectors/collect-recession.mjs'; throw err;
    }
    throw e;
  }
  let parsed;
...
function extractSeries(doc) {
  ...
  const series = arr.map(r => {
    const v = Number(r.value ?? r.probability ?? r.prob);
    return {
...
function computeStats(rows) {
  ...
  // Дней выше 50% (порог высокой вероятности)
  ...
  return {
    ...
    last_tierLabel: last?.tier.label ?? null,
...
export async function handler(req, res) {
  try {
    ...
    const sub = urlObj.pathname.replace(/^\/api\/layers\/recession/, '') || '/';
    ...
    if (sub === '/composite') {
      ...
      // Композит: последнее значение, тир, тренд, волатильность, превышение порогов
      return sendJSON(res, 200, {
        composite: {
          value: stats.last_value,
          tier: stats.last_tier,
          tierLabel: stats.last_tierLabel,
          date: stats.last_date,
          change_pct: stats.change_pct,
          trend_7v7_pct: stats.trend_7v7_pct,
          stddev: stats.stddev,
          range: { min: stats.min, max: stats.max },
          above_50_days: stats.above_50,
          above_70_days: stats.above_70,
        }
      }, extra);
    }
    if (sub === '/series') {
