/**
 * apis/sources/notam-api.mjs — API-МОДУЛЬ: NOTAM
 *
 * КОНТРАКТ CRUCIX v2.
 * ИСТОЧНИК: data/basket/notam.json — массив { id, text, country, lat, lng, severity }.
 * Сборщик: scripts/collectors/collect-notam.mjs.
 *
 * NOTAM (Notice to Airmen) — извещения о закрытии воздушного пространства, военных учениях, опасных зонах.
 * Severity: 1-5 (чем выше, тем критичнее).
 *
 * ФОРМАТЫ: json, csv, stats, raw.
 * ФИЛЬТРЫ: ?country=, ?min_severity=, ?max_severity=, ?q=, ?limit=.
 */

import { promises as fs } from 'fs';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';

const __filename = fileURLToPath(import.meta.url);
const __dirname  = dirname(__filename);
const PROJECT_ROOT = join(__dirname, '..', '..');
const BASKET_FILE  = join(PROJECT_ROOT, 'data', 'basket', 'notam.json');

export const route  = '/api/layers/notam';
export const method = 'GET';

export const meta = {
  category: 'transport',
  icon: '✈️',
  color: '#ff4444',
  vizType: 'marker',
  source: 'basket/notam.json',
  collector: 'collect-notam.mjs',
  cache: 300,
  description: 'NOTAM — извещения о воздушных зонах, учениях, ограничениях',
  unit: 'notices',
};

function severityLevel(sev) {
  if (sev >= 5) return { level: 'critical', color: '#7f1d1d', label: 'Критический' };
  if (sev >= 4) return { level: 'high',     color: '#dc2626', label: 'Высокий' };
  if (sev >= 3) return { level: 'medium',   color: '#f97316', label: 'Средний' };
  if (sev >= 2) return { level: 'low',      color: '#eab308', label: 'Низкий' };
  return           { level: 'info',     color: '#22c55e', label: 'Информационный' };
}

async function loadNotams() {
  let raw;
  try { raw = await fs.readFile(BASKET_FILE, 'utf8'); }
  catch (e) {
    if (e.code === 'ENOENT') {
      const err = new Error('no_data'); err.statusCode = 503;
      err.hint = 'run scripts/collectors/collect-notam.mjs'; throw err;
    }
    throw e;
  }
  let parsed;
  try { parsed = JSON.parse(raw); }
  catch (e) { const err = new Error('invalid_json_in_basket: ' + e.message); err.statusCode = 500; throw err; }

  let arr = null;
  if (Array.isArray(parsed)) arr = parsed;
  else if (parsed && Array.isArray(parsed.notams)) arr = parsed.notams;
  else if (parsed && Array.isArray(parsed.data))   arr = parsed.data;
  if (!arr) { const err = new Error('unrecognized_basket_format'); err.statusCode = 500; throw err; }

  const clean = arr.map(r => ({
    id: r.id || 'NOTAM-???',
    text: r.text || r.description || '',
    country: r.country || null,
    lat: Number(r.lat ?? r.latitude),
    lng: Number(r.lng ?? r.lon ?? r.longitude),
    severity: Number(r.severity ?? 1),
    type: r.type || 'airspace',
    validFrom: r.validFrom || r.from || null,
    validTo: r.validTo || r.to || null,
  })).filter(r => Number.isFinite(r.lat) && Number.isFinite(r.lng));

  if (clean.length === 0) { const err = new Error('empty_after_normalize'); err.statusCode = 500; throw err; }
  clean.sort((a, b) => b.severity - a.severity);
  return clean;
}

function applyFilters(rows, query) {
  let r = rows.slice();
  if (query.country) { const c = String(query.country).toLowerCase(); r = r.filter(x => (x.country || '').toLowerCase().includes(c)); }
  if (query.type) { const t = String(query.type).toLowerCase(); r = r.filter(x => x.type.toLowerCase() === t); }
  if (query.min_severity != null) { const n = parseInt(query.min_severity, 10); if (Number.isFinite(n)) r = r.filter(x => x.severity >= n); }
  if (query.max_severity != null) { const n = parseInt(query.max_severity, 10); if (Number.isFinite(n)) r = r.filter(x => x.severity <= n); }
  if (query.q) { const q = String(query.q).toLowerCase(); r = r.filter(x => x.text.toLowerCase().includes(q)); }
  if (query.limit) { const n = parseInt(query.limit, 10); if (n > 0) r = r.slice(0, n); }
  return r;
}

function computeStats(rows) {
  if (rows.length === 0) return { count: 0 };
  const bySeverity = {}, byCountry = {}, byType = {};
  let totalSeverity = 0;
  for (const r of rows) {
    const lvl = severityLevel(r.severity).level;
    bySeverity[lvl] = (bySeverity[lvl] || 0) + 1;
    if (r.country) byCountry[r.country] = (byCountry[r.country] || 0) + 1;
    byType[r.type] = (byType[r.type] || 0) + 1;
    totalSeverity += r.severity;
  }
  const top_countries = Object.entries(byCountry).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([name, count]) => ({ name, count }));
  return {
    count: rows.length,
    avg_severity: +(totalSeverity / rows.length).toFixed(2),
    max_severity: Math.max(...rows.map(r => r.severity)),
    by_severity: bySeverity,
    by_country: byCountry,
    by_type: byType,
    top_countries,
  };
}

function toFeatureCollection(rows) {
  const features = rows.map(r => {
    const s = severityLevel(r.severity);
    return {
      type: 'Feature',
      geometry: { type: 'Point', coordinates: [r.lng, r.lat] },
      properties: {
        id: r.id, text: r.text, country: r.country, type: r.type,
        severity: r.severity, severityLevel: s.level, severityLabel: s.label,
        validFrom: r.validFrom, validTo: r.validTo,
        color: s.color,
        category: 'transport', icon: meta.icon,
      },
    };
  });
  return {
    type: 'FeatureCollection',
    legend: [
      { level: 'info',     label: 'Информационный (1)', color: '#22c55e' },
      { level: 'low',      label: 'Низкий (2)',         color: '#eab308' },
      { level: 'medium',   label: 'Средний (3)',        color: '#f97316' },
      { level: 'high',     label: 'Высокий (4)',        color: '#dc2626' },
      { level: 'critical', label: 'Критический (5+)',   color: '#7f1d1d' },
    ],
    features,
  };
}

function envelopeMeta(full, filtered) {
  return {
    source: meta.source, collector: meta.collector, category: meta.category, unit: meta.unit,
    updated_at: new Date().toISOString(),
    total_notams: full.length, returned_notams: filtered.length,
  };
}

function sendJSON(res, status, payload, extra = {}) {
  const body = JSON.stringify(payload);
  res.writeHead(status, { 'Content-Type': 'application/json; charset=utf-8', 'Content-Length': String(Buffer.byteLength(body)), ...extra });
  res.end(body);
}
function sendText(res, status, text, ct = 'text/plain; charset=utf-8') {
  res.writeHead(status, { 'Content-Type': ct, 'Content-Length': String(Buffer.byteLength(text)) });
  res.end(text);
}
function toCSVBody(rows) {
  const lines = ['id,text,country,type,severity,severityLevel,lat,lng'];
  for (const r of rows) { const s = severityLevel(r.severity); lines.push(`${r.id},"${r.text.replace(/"/g, '""')}",${r.country || ''},${r.type},${r.severity},${s.level},${r.lat},${r.lng}`); }
  return lines.join('\n') + '\n';
}

export async function handler(req, res) {
  try {
    const urlObj = new URL(req.url, 'http://x');
    const query = Object.fromEntries(urlObj.searchParams.entries());
    const format = (query.format || 'json').toLowerCase();

    const full = await loadNotams();
    const rows = applyFilters(full, query);
    const stats = computeStats(rows);
    const extra = { 'X-Module': 'notam-api', 'X-Module-Version': '2.0.0', 'Cache-Control': `public, max-age=${meta.cache}` };

    if (format === 'csv') return sendText(res, 200, toCSVBody(rows), 'text/csv; charset=utf-8');
    if (format === 'stats') return sendJSON(res, 200, { stats, meta: envelopeMeta(full, rows) }, extra);
    if (format === 'raw') return sendJSON(res, 200, { data: rows, meta: envelopeMeta(full, rows) }, extra);

    const fc = toFeatureCollection(rows);
    return sendJSON(res, 200, {
      type: 'FeatureCollection',
      meta: envelopeMeta(full, rows),
      legend: fc.legend,
      features: fc.features,
      stats,
    }, extra);

  } catch (e) {
    const status = e.statusCode || 500;
    const payload = { error: status === 503 ? 'no_data' : 'handler_error', message: e.message };
    if (e.hint) payload.hint = e.hint;
    try { sendJSON(res, status, payload); } catch {}
  }
}
