...
export async function briefing() {
  try {
    const response = await fetch('https://cbr.ru/rss/RssNews', {
      headers: {
    ...
    console.error(`[ЦентрБанк] Загружено ${items.length} новостей`);
    ...
    console.error('[ЦентрБанк] Ошибка:', error.message);
    return {
      source: 'ЦентрБанк',
      timestamp: new Date().toISOString(),
...
if (process.argv[1]?.endsWith('cbr.mjs')) {
