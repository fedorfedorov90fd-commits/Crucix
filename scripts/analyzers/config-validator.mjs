...
import ConfigValidator from '../../apis/sources/config-validator.mjs';
const __dirname = dirname(fileURLToPath(import.meta.url));
const ROOT = join(__dirname, '..', '..');
const OUT_DIR = join(ROOT, 'data', 'analytics', 'specialist');
const OUT_FILE = join(OUT_DIR, 'config-validator.json');
async function main() {
  ...
  const payload = {
    _meta: { id: 'config-validator', category: 'specialist', version: '1.0.0', schema_version: '1.0.0', sources: ['self'], calculator: 'ConfigValidator', updated_at: now, duration_ms: Date.now() - t0, checksum: '', stats },
  ...
  console.log('config-validator v1.0.0 — stats:', JSON.stringify(stats));
}
