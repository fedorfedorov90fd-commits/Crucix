/**
 * apis/sources/ovx-api.mjs — API-МОДУЛЬ: ВОЛАТИЛЬНОСТЬ НЕФТИ (OVX)
 *
 * КОНТРАКТ CRUCIX v2.
 * ИСТОЧНИК: data/basket/ovx.json — [{ date, value }] ИЛИ { source, lastUpdated, data:[{date,value}] }.
 * Сборщик: scripts/collectors/collect-ovx.mjs.
 *
 * OVX (CBOE Crude Oil Volatility Index) — «VIX для нефти». Индикатор
 * волатильности нефтяного рынка. Рост OVX = страх и нестабильность.
 *
...
const BASKET_FILE  = join(PROJECT_ROOT, 'data', 'basket', 'ovx.json');

export const route  = '/api/layers/ovx';
...
export const meta = {
  ...
  color: '#ff6600',
  vizType: 'series',
  source: 'basket/ovx.json',
  collector: 'collect-ovx.mjs',
  cache: 300,
  description: 'OVX — волатильность нефти (CBOE Crude Oil Volatility Index)',
  unit: 'index',
};

const TIERS = {
  panic:    { min: 60, color: '#dc2626', label: 'Паника' },
  stress:   { min: 45, color: '#f97316', label: 'Стресс' },
  elevated: { min: 32, color: '#eab308', label: 'Повышенная' },
  normal:   { min: 20, color: '#84cc16', label: 'Норма' },
  calm:     { min: 0,  color: '#22c55e', label: 'Спокойствие' },
};
...
async function loadData() {
  ...
  try { raw = await fs.readFile(BASKET_FILE, 'utf8'); }
    ...
    if (e.code === 'ENOENT') {
      ...
      err.hint = 'run scripts/collectors/collect-ovx.mjs'; throw err;
    }
    throw e;
  }
  let parsed;
...
function extractSeries(doc) {
  ...
  const series = arr.map(r => {
    const v = Number(r.value ?? r.ovx ?? r.close);
    return {
...
export async function handler(req, res) {
  try {
    ...
    const sub = urlObj.pathname.replace(/^\/api\/layers\/ovx/, '') || '/';
    ...
    if (sub === '/composite') {
      // Композит: последнее значение + тир + изменение + волатильность
