...
import RateLimiter from '../../apis/sources/rate-limiter.mjs';
const __dirname = dirname(fileURLToPath(import.meta.url));
const ROOT = join(__dirname, '..', '..');
const OUT_DIR = join(ROOT, 'data', 'analytics', 'specialist');
const OUT_FILE = join(OUT_DIR, 'rate-limiter.json');
async function main() {
  ...
  const payload = {
    _meta: { id: 'rate-limiter', category: 'specialist', version: '1.0.0', schema_version: '1.0.0', sources: ['self'], calculator: 'RateLimiter', updated_at: now, duration_ms: Date.now() - t0, checksum: '', stats },
  ...
  console.log('rate-limiter v1.0.0 — stats:', JSON.stringify(stats));
}
