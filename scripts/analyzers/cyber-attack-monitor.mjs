...
import CyberAttackMonitor from '../../apis/sources/cyber-attack-monitor.mjs';
const __dirname = dirname(fileURLToPath(import.meta.url));
const ROOT = join(__dirname, '..', '..');
const BASKET = join(ROOT, 'data', 'basket');
const OUT_DIR = join(ROOT, 'data', 'analytics', 'detector');
const OUT_FILE = join(OUT_DIR, 'cyber-attack-monitor.json');
async function readJson(p, fb = null) { try { return JSON.parse(await readFile(p, 'utf-8')); } catch { return fb; } }
async function main() {
  const now = new Date().toISOString();
  const t0 = Date.now();
  const instance = new CyberAttackMonitor();
  const data = await readJson(join(BASKET, 'cyber-attacks.json'), null);
  ...
  const payload = {
    _meta: { id: 'cyber-attack-monitor', category: 'detector', version: '1.0.0', schema_version: '1.0.0', sources: ['cyber-attacks.json'], calculator: 'CyberAttackMonitor', updated_at: now, duration_ms: Date.now() - t0, checksum: '', stats: { items_processed: count, total_result: all.length } },
  ...
  console.log('cyber-attack-monitor v1.0.0 — items:', count, ', result:', all.length);
}
