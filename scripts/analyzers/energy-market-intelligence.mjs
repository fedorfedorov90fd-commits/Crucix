...
import EnergyMarketIntelligence from '../../apis/sources/energy-market-intelligence.mjs';
const __dirname = dirname(fileURLToPath(import.meta.url));
const ROOT = join(__dirname, '..', '..');
const BASKET = join(ROOT, 'data', 'basket');
const OUT_DIR = join(ROOT, 'data', 'analytics', 'market');
const OUT_FILE = join(OUT_DIR, 'energy-market-intelligence.json');
async function readJson(p, fb = null) { try { return JSON.parse(await readFile(p, 'utf-8')); } catch { return fb; } }
async function main() {
  const now = new Date().toISOString();
  const t0 = Date.now();
  const instance = new EnergyMarketIntelligence();
  const data = await readJson(join(BASKET, 'oil-gas.json'), null);
  ...
  const payload = {
    _meta: { id: 'energy-market-intelligence', category: 'market', version: '1.0.0', schema_version: '1.0.0', sources: ['oil-gas.json'], calculator: 'EnergyMarketIntelligence', updated_at: now, duration_ms: Date.now() - t0, checksum: '', stats: { items_processed: count, total_result: all.length } },
  ...
  console.log('energy-market-intelligence v1.0.0 — items:', count, ', result:', all.length);
}
