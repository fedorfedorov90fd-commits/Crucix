...
async function testFetch() {
    ...
    try {
        ...
        if (response.data && response.data.observations) {
            ...
            // Если есть существующие данные, используем их историю
            ...
            const testData = {
                ...
                metadata: {
                    source: 'FRED API',
                    dataPoints: data.length || 2,
                    lastUpdate: new Date().toISOString()
                },
                metrics: {
                    goldPrice: data[0]?.value ? parseFloat(data[0].value) : 1950.40,
                    oilPrice: 79.40,
                    ratioChange1d: 0.15,
                    ratioChange7d: 0.38
                }
