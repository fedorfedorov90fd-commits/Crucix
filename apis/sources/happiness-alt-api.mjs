...
const BASKET_PATH = join(__dirname, '..', '..', 'data', 'basket', 'happiness-alt.json');
...
export async function handleHappinessAltAPI(req, res) {
    const url = new URL(req.url, `http://${req.headers.host}`);
    const pathname = url.pathname;

    if (pathname === '/api/happiness-alt/' || pathname === '/api/happiness-alt') {
    ...
    if (pathname === '/api/happiness-alt/status') {
        const data = await loadData();
        res.writeHead(200, { 'Content-Type': 'application/json' });
