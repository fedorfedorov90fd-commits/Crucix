...
import SocialSentimentAnalyzer from '../../apis/sources/social-sentiment-analyzer.mjs';
const __dirname = dirname(fileURLToPath(import.meta.url));
const ROOT = join(__dirname, '..', '..');
const BASKET = join(ROOT, 'data', 'basket');
const OUT_DIR = join(ROOT, 'data', 'analytics', 'semantic');
const OUT_FILE = join(OUT_DIR, 'social-sentiment-analyzer.json');
async function readJson(p, fb = null) { try { return JSON.parse(await readFile(p, 'utf-8')); } catch { return fb; } }
async function main() {
  const now = new Date().toISOString();
  const t0 = Date.now();
  const instance = new SocialSentimentAnalyzer();
  const data = await readJson(join(BASKET, 'news.json'), null);
  ...
  const payload = {
    _meta: { id: 'social-sentiment-analyzer', category: 'semantic', version: '1.0.0', schema_version: '1.0.0', sources: ['news.json'], calculator: 'SocialSentimentAnalyzer', updated_at: now, duration_ms: Date.now() - t0, checksum: '', stats: { items_processed: count, total_result: all.length } },
  ...
  console.log('social-sentiment-analyzer v1.0.0 — items:', count, ', result:', all.length);
}
