/**
 * apis/sources/rate-limiter-api.mjs — SERVICE-МОДУЛЬ: RATE LIMITER
 *
 * КОНТРАКТ CRUCIX v2 (SERVICE).
 * ИСТОЧНИК: data/analytics/specialist/rate-limiter.json — { _meta: { id, category, version, calculator, updated_at, duration_ms, checksum, stats }, data: { items: [...], extra: {...} } }.
 * НАЗНАЧЕНИЕ: телеметрия rate limiter-а — лимиты, окна, статистика срабатываний.
 *
 * ЭНДПОИНТЫ:
 *   GET /             — полный документ
 *   GET /stats        — _meta + extra
 *   GET /items        — данные items
 *   GET /items/:id    — один item
 *   GET /limits       — (если есть) конфигурация лимитов
...
const FILE = join(PROJECT_ROOT, 'data', 'analytics', 'specialist', 'rate-limiter.json');

export const route  = '/api/services/rate-limiter';
export const method = 'GET';

export const meta = {
  service: true,
  description: 'Rate limiter telemetry service: exposes limits, windows, hit counts from data/analytics/specialist/rate-limiter.json.',
  cache: 300,
  version: '2.0.0',
};
...
async function loadData() {
  ...
  try { raw = await fs.readFile(FILE, 'utf8'); }
    ...
    if (e.code === 'ENOENT') { const err = new Error('not_generated'); err.statusCode = 503; err.hint = 'run scripts/analyzers/rate-limiter.mjs'; throw err; }
    throw e;
  }
  let parsed;
...
async function epItems(doc) { const items = doc.data.items || []; return { items, total: items.length }; }
...
async function epLimits(doc) {
  const v = doc.data.limits || doc.data.config || null;
  if (v === null) { const e = new Error('field_absent'); e.statusCode = 404; e.field = 'limits'; throw e; }
  return { limits: v };
}
...
export async function handler(req, res) {
  ...
  const subPath = url.pathname.replace(/^\/api\/services\/rate-limiter/, '') || '/';
  ...
  try {
    ...
    else if (segs[0] === 'limits') result = await epLimits(doc);
