...
import CentralBankPredictor from '../../apis/sources/central-bank-predictor.mjs';
const __dirname = dirname(fileURLToPath(import.meta.url));
const ROOT = join(__dirname, '..', '..');
const BASKET = join(ROOT, 'data', 'basket');
const OUT_DIR = join(ROOT, 'data', 'analytics', 'forecast');
const OUT_FILE = join(OUT_DIR, 'central-bank-predictor.json');
async function readJson(p, fb = null) { try { return JSON.parse(await readFile(p, 'utf-8')); } catch { return fb; } }
async function main() {
  const now = new Date().toISOString();
  const t0 = Date.now();
  const instance = new CentralBankPredictor();
  const data = await readJson(join(BASKET, 'fred.json'), null);
  ...
  const payload = {
    _meta: { id: 'central-bank-predictor', category: 'forecast', version: '1.0.0', schema_version: '1.0.0', sources: ['fred.json'], calculator: 'CentralBankPredictor', updated_at: now, duration_ms: Date.now() - t0, checksum: '', stats: { items_processed: count, total_result: all.length } },
  ...
  console.log('central-bank-predictor v1.0.0 — items:', count, ', result:', all.length);
}
