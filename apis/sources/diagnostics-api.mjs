...
// ============================================================
// МОДУЛЬ №46: САМОДИАГНОСТИКА СИСТЕМЫ (SYSTEM HEALTH & DIAGNOSTICS)
// ============================================================
// Анализирует состояние всех модулей Crucix:
//   1. Статус модулей (онлайн/офлайн)
//   2. Производительность (время ответа)
...
class DiagnosticsManager {
    ...
    async runFullDiagnostics() {
        console.log('[Diagnostics] Запуск полной диагностики...');
    ...
    async checkDataIntegrity() {
        ...
        try {
            ...
            if (content.includes('<opml') && content.includes('xmlUrl')) {
                ...
                results.opml = { status: 'CORRUPTED', error: 'Невалидный OPML' };
            }
        } catch (e) {
    ...
    async analyzeLogsWithAI() {
        try {
            ...
            if (logEntries.length === 0) {
                return { status: 'NO_LOGS', message: 'Логи для анализа не найдены' };
            }
...
Задачи:
1. Определи, есть ли в логах ошибки или предупреждения.
2. Оцени общее состояние системы по логам (стабильно/есть проблемы/критично).
3. Дай краткие рекомендации по устранению проблем (если они есть).

Ответь в формате JSON:
{
  "status": "STABLE" | "WARNING" | "CRITICAL",
  "summary": "краткое резюме состояния (1-2 предложения)",
  "issues": ["список обнаруженных проблем (если есть)"],
  "recommendations": ["список рекомендаций (если есть)"]
}`;
    ...
    async saveReport() {
        try {
            ...
            console.error('[Diagnostics] Ошибка сохранения отчёта:', e.message);
        }
    }
    ...
    getStatusText() {
        ...
        const statusMap = {
            'ONLINE': '🟢 СИСТЕМА РАБОТАЕТ НОРМАЛЬНО',
    ...
    try {
        ...
        if (path === '/api/diagnostics/latest' && req.method === 'GET') {
            ...
            if (report) {
                ...
                res.end(JSON.stringify({
                    success: false,
                    error: 'Отчёт не найден. Запустите диагностику.'
                }));
            }
            return;
        }
