// Aeon — философия, культура, идеи
export async function briefing() {
  try {
    const response = await fetch('https://aeon.co/feed.rss', {
      headers: {
    ...
    console.error(`[Aeon] Загружено ${items.length} новостей`);
    ...
    console.error('[Aeon] Ошибка:', error.message);
    return {
      source: 'Aeon',
      timestamp: new Date().toISOString(),
...
if (process.argv[1]?.endsWith('aeon.mjs')) {
