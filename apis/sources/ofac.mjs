  ...
  try {
    ...
    if (path === '/api/ofac/status' && req.method === 'GET') {
      ...
      res.end(JSON.stringify({
        ...
        module: 'ofac',
        status: 'online',
        entities: basket.data.entities?.length || 0,
    ...
    if (path === '/api/ofac/data' && req.method === 'GET') {
      res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    res.end(JSON.stringify({
      success: false,
      error: 'Нет данных. Запустите: node scripts/collect-ofac.mjs'
    }));
    return;
  }
