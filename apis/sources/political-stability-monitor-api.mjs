/**
 * apis/sources/political-stability-monitor-api.mjs — API-МОДУЛЬ: МОНИТОР ПОЛИТИЧЕСКОЙ СТАБИЛЬНОСТИ
 *
 * КОНТРАКТ CRUCIX v2.
 * ИСТОЧНИК: data/analytics/specialist/political-stability-monitor.json.
 * Анализатор: scripts/analyzers/political-stability-monitor.mjs.
 *
 * Политическая стабильность стран: индексы, риски, тренды.
 */
...
const ANALYTICS_FILE = join(PROJECT_ROOT, 'data', 'analytics', 'specialist', 'political-stability-monitor.json');

export const route  = '/api/layers/political-stability-monitor';
...
export const meta = {
  ...
  color: '#0088ff',
  vizType: 'marker',
  source: 'analytics/specialist/political-stability-monitor.json',
  collector: 'analyzer:political-stability-monitor',
  cache: 300,
  description: 'Монитор политической стабильности стран',
  unit: 'index',
};

function stabilityBand(value) {
  if (value >= 80) return { level: 'very_high', color: '#22c55e', label: 'Очень высокая' };
  if (value >= 60) return { level: 'high',      color: '#84cc16', label: 'Высокая' };
  if (value >= 40) return { level: 'medium',    color: '#eab308', label: 'Средняя' };
  if (value >= 20) return { level: 'low',       color: '#f97316', label: 'Низкая' };
  return                 { level: 'critical',  color: '#dc2626', label: 'Критическая' };
}
...
async function loadData() {
  ...
  try { raw = await fs.readFile(ANALYTICS_FILE, 'utf8'); }
    ...
    if (e.code === 'ENOENT') {
      ...
      err.hint = 'run scripts/analyzers/political-stability-monitor.mjs'; throw err;
    }
    throw e;
  }
...
function normalizeItem(r) {
  return {
    ...
    score: Number(r.score ?? r.stability ?? r.value ?? 0),
...
function computeStats(rows) {
  ...
  const byBand = {};
  for (const r of rows) { const b = stabilityBand(r.score).level; byBand[b] = (byBand[b] || 0) + 1; }
...
function toFeatureCollection(rows) {
  const features = rows.filter(r => r.lat !== 0 || r.lng !== 0).map(r => {
    const b = stabilityBand(r.score);
    return {
      type: 'Feature',
      geometry: { type: 'Point', coordinates: [r.lng, r.lat] },
      properties: {
        id: r.id, name: r.name, iso: r.iso,
        score: r.score, risk: r.risk, trend: r.trend, severity: r.severity, date: r.date,
  ...
  return {
    ...
    bands: [
      { level: 'very_high', label: 'Очень высокая (80+)', color: '#22c55e' },
      { level: 'high',      label: 'Высокая (60+)',        color: '#84cc16' },
      { level: 'medium',    label: 'Средняя (40+)',        color: '#eab308' },
      { level: 'low',       label: 'Низкая (20+)',         color: '#f97316' },
      { level: 'critical',  label: 'Критическая (<20)',    color: '#dc2626' },
    ],
    features,
  };
}
...
function toCSVBody(rows) {
  const lines = ['iso,name,score,risk,band,trend'];
  for (const r of rows) { const b = stabilityBand(r.score); lines.push(`${r.iso || ''},"${r.name.replace(/"/g, '""')}",${r.score},${r.risk},${b.level},${r.trend || ''}`); }
  return lines.join('\n') + '\n';
}
...
export async function handler(req, res) {
  try {
    ...
    const extra = { 'X-Module': 'political-stability-monitor-api', 'X-Module-Version': '2.0.0', 'Cache-Control': `public, max-age=${meta.cache}` };
