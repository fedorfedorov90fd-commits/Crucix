...
export default async function handler(req, res) {
  try {
    ...
    const econKeywords = ['economy', 'gdp', 'recession', 'inflation', 'trade', 'export', 'import', 'sanction', 'crisis', 'market'];
    const econNews = news.filter(n => {
      const text = (n.title + ' ' + n.description).toLowerCase();
      return econKeywords.some(kw => text.includes(kw));
    });
    
    const features = econNews.slice(0, 50).map(n => ({
      type: 'Feature',
      geometry: { type: 'Point', coordinates: [n.lng || 0, n.lat || 0] },
      properties: {
        value: 1,
        label: n.title || 'Economic event',
