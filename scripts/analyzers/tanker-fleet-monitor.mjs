...
import TankerFleetMonitor from '../../apis/sources/tanker-fleet-monitor.mjs';
const __dirname = dirname(fileURLToPath(import.meta.url));
const ROOT = join(__dirname, '..', '..');
const BASKET = join(ROOT, 'data', 'basket');
const OUT_DIR = join(ROOT, 'data', 'analytics', 'flow');
const OUT_FILE = join(OUT_DIR, 'tanker-fleet-monitor.json');
async function readJson(p, fb = null) { try { return JSON.parse(await readFile(p, 'utf-8')); } catch { return fb; } }
async function main() {
  const now = new Date().toISOString();
  const t0 = Date.now();
  const instance = new TankerFleetMonitor();
  const data = await readJson(join(BASKET, 'dark-fleet.json'), null);
  ...
  const payload = {
    _meta: { id: 'tanker-fleet-monitor', category: 'flow', version: '1.0.0', schema_version: '1.0.0', sources: ['dark-fleet.json'], calculator: 'TankerFleetMonitor', updated_at: now, duration_ms: Date.now() - t0, checksum: '', stats: { items_processed: count, total_result: all.length } },
  ...
  console.log('tanker-fleet-monitor v1.0.0 — items:', count, ', result:', all.length);
}
