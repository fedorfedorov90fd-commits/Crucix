/**
 * apis/sources/space-api.mjs — API-МОДУЛЬ: КОСМИЧЕСКИЙ МОНИТОРИНГ
 *
 * КОНТРАКТ CRUCIX v2.
 * ИСТОЧНИК: data/basket/space.json — массив { id, name, lat, lng, severity, timestamp }.
 * Сборщик: scripts/collectors/collect-space-track.mjs.
 *
 * Космические события: запуски, спутники, орбитальные объекты.
 */
...
const BASKET_FILE  = join(PROJECT_ROOT, 'data', 'basket', 'space.json');

export const route  = '/api/layers/space';
...
export const meta = {
  ...
  color: '#6600cc',
  vizType: 'marker',
  source: 'basket/space.json',
  collector: 'collect-space-track.mjs',
  cache: 300,
  description: 'Космические события: запуски, спутники, орбитальные объекты',
  unit: 'objects',
};
...
function computeStats(rows) {
  ...
  return { count: rows.length, by_severity: bySeverity, by_type: byType, by_country: byCountry, top_countries };
}
...
function toFeatureCollection(rows) {
  const features = rows.map(r => ({
    ...
    properties: {
      ...
      country: r.country, orbit: r.orbit, operator: r.operator, timestamp: r.timestamp,
...
function envelopeMeta(full, filtered) {
  return {
    ...
    total_objects: full.length, returned_objects: filtered.length,
  };
}
...
function toCSVBody(rows) {
  const lines = ['id,name,type,severity,country,orbit,operator,lat,lng'];
  for (const r of rows) lines.push(`${r.id},"${r.name.replace(/"/g, '""')}",${r.type},${r.severity},${r.country || ''},${r.orbit || ''},${r.operator || ''},${r.lat},${r.lng}`);
  return lines.join('\n') + '\n';
}
...
export async function handler(req, res) {
  try {
    ...
    const extra = { 'X-Module': 'space-api', 'X-Module-Version': '2.0.0', 'Cache-Control': `public, max-age=${meta.cache}` };
