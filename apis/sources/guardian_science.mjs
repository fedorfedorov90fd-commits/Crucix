...
export async function briefing() {
  try {
    const response = await fetch('https://www.theguardian.com/science/rss', {
      headers: {
    ...
    console.error(`[The Guardian (Наука)] Загружено ${items.length} новостей`);
    ...
    console.error('[The Guardian (Наука)] Ошибка:', error.message);
    return {
      source: 'The Guardian (Наука)',
      timestamp: new Date().toISOString(),
...
if (process.argv[1]?.endsWith('guardian_science.mjs')) {
