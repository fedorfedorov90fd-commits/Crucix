...
const BASKET_PATH = join(__dirname, '..', '..', 'data', 'basket', 'vxx.json');
...
export async function handleVXXAPI(req, res) {
    const url = new URL(req.url, `http://${req.headers.host}`);
    const pathname = url.pathname;

    if (pathname === '/api/vxx/' || pathname === '/api/vxx') {
