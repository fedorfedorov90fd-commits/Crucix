/**
 * apis/sources/business-optimism-api.mjs — API-МОДУЛЬ: ИНДЕКС ДЕЛОВОГО ОПТИМИЗМА
 *
 * КОНТРАКТ CRUCIX v2.
 * ИСТОЧНИК: data/basket/business-optimism.json — [{ date, value }] ИЛИ { source, lastUpdated, data:[{date,value}], meta }.
 * Сборщик: scripts/collectors/collect-business-optimism.mjs. FRED / Conference Board.
 *
 * Индекс делового оптимизма — опережающий индикатор экономической активности.
 *
...
const BASKET_FILE  = join(PROJECT_ROOT, 'data', 'basket', 'business-optimism.json');

export const route  = '/api/layers/business-optimism';
...
export const meta = {
  ...
  color: '#0891b2',
  vizType: 'series',
  source: 'basket/business-optimism.json',
  collector: 'collect-business-optimism.mjs',
  cache: 300,
  description: 'Индекс делового оптимизма (FRED / Conference Board)',
  unit: 'index',
};
...
async function loadData() {
  ...
  try { raw = await fs.readFile(BASKET_FILE, 'utf8'); }
    ...
    if (e.code === 'ENOENT') {
      ...
      err.hint = 'run scripts/collectors/collect-business-optimism.mjs'; throw err;
    }
    throw e;
  }
  let parsed;
...
export async function handler(req, res) {
  try {
    ...
    const sub = urlObj.pathname.replace(/^\/api\/layers\/business-optimism/, '') || '/';
    ...
    const extra = {
      'X-Module': 'business-optimism-api',
    ...
    if (sub === '/status') {
      return sendJSON(res, 200, { status: 'online', count: series.length, source }, extra);
    }
